...
# Kategori → slug sırası. Yeni eklenen belgeler otomatik "Diğer"e düşer.
CATS = [
...
extra = sorted(
    re.match(r"pediatrik-(.+)-algoritmasi\.html$", f).group(1)
    for f in os.listdir(D)
    if re.match(r"pediatrik-(.+)-algoritmasi\.html$", f) and "yol-haritasi" not in f
    and re.match(r"pediatrik-(.+)-algoritmasi\.html$", f).group(1) not in known)
cats = list(CATS)
if extra:
    cats.append(("Diğer / yeni","diger",extra))
...
for cat_title, cat_key, rows in grouped:
    for slug, title, sub, body in rows:
        hay = html.escape((title+" "+sub).lower())
        cards.append(f'''<a class="algo" href="pediatrik-{slug}-algoritmasi.html" data-cat="{cat_key}" data-search="{hay}">
...
chips = '<button class="chip active" data-filter="all">Tümü</button>' + "".join(
    f'<button class="chip" data-filter="{k}">{html.escape(short(t))}</button>' for t,k,_ in grouped)
index_css = """
  *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:var(--sans);line-height:1.5}
  ...
  .algo[data-cat=goruntuleme]{--tone:#7c3aed}.algo[data-cat=hastalik]{--tone:#b91c1c}
...
<style>{STYLE_INNER}{index_css}</style></head><body><div class="wrap">
<header><p class="eyebrow">Klinik Karar Kütüphanesi</p><h1>Çocuk Gastroenterolojisi — Algoritma Kütüphanesi</h1>
<p class="sub">{total} uzman-düzeyi algoritma. Başlığa/belirtiye göre arayın; karta tıklayıp açın. <a href="app.html" style="color:var(--accent)">Tek sayfa uygulama →</a></p></header>
<div class="toolbar"><input class="search" id="q" type="search" placeholder="Ara: reflü, kanama, ALT, Wilson, pankreatit…" autocomplete="off"><div class="chips" id="chips">{chips}</div></div>
<div class="count" id="count"></div><div class="grid" id="grid">{"".join(cards)}</div>
...
function apply(){{const term=norm(q.value.trim());let n=0;cards.forEach(c=>{{const ok=(filter==='all'||c.dataset.cat===filter)&&(!term||norm(c.dataset.search).includes(term));c.style.display=ok?'':'none';if(ok)n++;}});count.textContent=n+' algoritma'+(term||filter!=='all'?' (filtreli)':'');empty.style.display=n?'none':'block';}}
...
for cat_title, cat_key, rows in grouped:
    ...
    for slug, title, sub, body in rows:
        hay = html.escape((title+" "+sub).lower())
        navhtml += f'<button class="navitem" data-slug="{slug}" data-search="{hay}"><span class="dot dot-{cat_key}"></span>{html.escape(title)}</button>'
        docshtml += f'<article class="doc" id="a-{slug}" data-cat="{cat_key}" hidden>{body}</article>'
    navhtml += '</div>'
appchips='<button class="chip active" data-filter="all">Tümü</button>'+"".join(f'<button class="chip" data-filter="{k}">{html.escape(t)}</button>' for k,t,_ in statmeta)
...
.side .top{padding:14px 14px 10px;border-bottom:1px solid var(--line);position:sticky;top:0;background:var(--surface);z-index:2}
...
.main{overflow:auto;min-height:0}.main .wrap{padding-top:22px}
...
@media print{.side,.menutoggle{display:none}.app{display:block;height:auto}.doc[hidden]{display:none}}
"""
...
<button class="menutoggle" id="menu">☰ Algoritmalar</button>
...
print(f"build.py OK — {total} algoritma · index.html + app.html güncellendi"
      + (f" · yeni/haritasız: {', '.join(extra)}" if extra else ""))
