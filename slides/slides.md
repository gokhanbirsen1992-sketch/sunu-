---
theme: seriph
title: Yıldız'ın Sarılığı
info: |
  ## Yıldız'ın Sarılığı
  Bir herediter hiperbilirubinemi anlatısı

  Uzm. Dr. Gökhan Birşen — Antalya Eğitim ve Araştırma Hastanesi
class: cover-bg
highlighter: shiki
lineNumbers: false
drawings:
...
<div class="flex gap-6 mt-12 opacity-50 text-xs tracking-wider">
  ...
  <div class="flex items-center gap-1"><mdi-heart-outline /> 1 vaka</div>
</div>
...
# <mdi-emoticon-happy class="inline text-yellow-500" /> Yıldız — 4 günlük
...
flowchart LR
    A[Yıldız patlamaları<br/>milyarlarca yıl] --> B[Atomlar yağmurlandı]
    B --> C[Kayalar, denizler]
    C --> D[Evrim, organizmalar]
    D --> E[Yıldız doğdu<br/>karaciğer, kemik iliği]
    E --> F[Eritrosit<br/>120 gün dolaşım]
    F --> G[Makrofaj fagositozu<br/>Kupffer hücreleri]
    G --> H[Hem oksigenaz<br/>CO + Fe + biliverdin]
    H --> I[Biliverdin redüktaz<br/>BİLİRUBİN]
    style I fill:#f5b400,stroke:#d97706,stroke-width:3px,color:#fff
```
...
```mermaid {scale: 0.6}
flowchart TD
    A[1. ÜRETİM<br/>Hem oksigenaz<br/>Biliverdin redüktaz] --> B[2. UPTAKE<br/>OATP1B1/B3<br/><i>bazolateral</i>]
    B --> C[3. KONJUGASYON<br/>UGT1A1<br/><i>endoplazmik retikulum</i>]
    C --> D[4. EFLÜKS<br/>MRP2<br/><i>apikal</i>]
    D --> E[5. ATILIM<br/>Safra → bağırsak]
    style A fill:#fef3c7,stroke:#92400e
    style B fill:#dcfce7,stroke:#166534
    style C fill:#fee2e2,stroke:#991b1b
    style D fill:#e0e7ff,stroke:#3730a3
    style E fill:#fce7f3,stroke:#9f1239
```
...
```mermaid {scale: 0.7}
flowchart LR
    F[Fetüs<br/>UGT1A1 ~0] -.bilirubin.-> P[Plasenta]
    F -.X biliverdin.-> P
    P --> M[Anne<br/>dolaşımı]
    M --> AK[Anne<br/>karaciğeri]
    style F fill:#fce7f3
    style M fill:#dcfce7
```
...
```mermaid {scale: 0.7}
flowchart TD
    PV[Portal ven] --> S[Sinüzoid]
    HA[Hepatik arter] --> S
    S --> D[Disse boşluğu]
    D --> H[Hepatosit<br/>bazolateral]
    H --> Hint[hücre içi]
    Hint --> K[Kanalikül<br/>apikal]
    K --> Her[Hering kanalı]
    Her --> Du[Duktul]
    Du --> Ko[Koledok]
    Ko --> Bag[Bağırsak]
...
```mermaid {scale: 0.75}
flowchart TD
    A[1 · Fraksiyonlama<br/>D/T oranı] --> A1{D/T &lt; %20?}
    A1 -->|evet — indirekt| B[2 · Üretim taraması<br/>CBC, retikülosit, Coombs, G6PD]
    A1 -->|%20-50| AT[Mikst:<br/>sepsis, hepatit]
    A1 -->|&gt;%50| AC[Konjuge:<br/>DJ, Rotor, atrezi]
    B --> B1{Hemoliz?}
    B1 -->|hayır| C[3 · Fototerapi yanıt nicelemesi<br/>Beklenen 30-50 mg/dL/24sa]
    C --> C1{Yanıt yetersiz?}
    C1 -->|evet| D[4 · Fenobarbital denemesi<br/>5 mg/kg/gün, 72 sa]
    D --> D1{Yanıt?}
    D1 -->|Evet %25-50| TII[CN tip II<br/>fenobarbital ile yönet]
    D1 -->|Hayır| TI[CN tip I<br/>karaciğer nakli yolu]
    D --> E[5 · Ebeveyn fenotipleme<br/>UGT1A1 sekanslama]
    E --> F[6 · Proband UGT1A1<br/>tam gen sekanslama]
    style A fill:#fef3c7
    style D fill:#fee2e2,stroke:#991b1b,stroke-width:3px
    style TI fill:#dc2626,color:#fff
    style TII fill:#f59e0b,color:#fff
```
...
<div class="flex gap-2"><mdi-heart-outline class="text-pink-500" /> Sonraki gebelik için prenatal tanı</div>
</div>
...
<div><mdi-emoticon-happy class="inline text-pink-500" /> <b>Moleküller</b> — plasentadan miras</div>
